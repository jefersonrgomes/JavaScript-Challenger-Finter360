...
for (i = 0; i < texto.length; i++) {
  ...
  console.log(`sorted String: ' ${temp}`);
  console.log('reconvertString: ' + Replaced(temp, an, a) + '\n');
  novoTexto.push(temp);
  novoTexto.sort();
}
console.log('novo texto: ' + novoTexto.toString());

function Replaced(val, org, rep) {
  for (let x = 0; x < val.length; x++) {
    let i = 0;
    while (i < 20) {
      //val[x] = val[x].replace(org[i], rep[i]);
      if (val[x] === org[i]) val[x] = rep[i];
      i++;
    }
  }
  return val;
